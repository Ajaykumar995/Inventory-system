...
export default function GlobalSearch() {
  ...
  return (
    <div ref={ref} className="relative w-full max-w-md">
      <input
        className="w-full px-4 py-2 rounded-xl glass border border-white/10 text-sm text-white placeholder-slate-500"
      ...
      {open && results.length > 0 && (
        <div className="absolute top-full mt-2 w-full glass rounded-xl border border-white/10 shadow-2xl z-50 overflow-hidden">
          {results.map((hit) => (
            <button
              key={`${hit.type}-${hit.id}`}
              onClick={() => pick(hit)}
              className="w-full px-4 py-3 flex items-center gap-3 text-left hover:bg-white/10 transition"
            >

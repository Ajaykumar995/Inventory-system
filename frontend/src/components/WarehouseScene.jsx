...
function StockBox({ item, position }) {
...
function WarehouseFloor() {
    return (<>
      <Grid position={[0, 0, 0]} args={[20, 20]} cellSize={0.5} cellThickness={0.5} cellColor="#1e3a5f" sectionSize={2} sectionThickness={1} sectionColor="#3b82f6" fadeDistance={25} infiniteGrid/>
...
function ShelvingUnit({ items, offsetX }) {
    return (<group position={[offsetX, 0, 0]}>
      {/* Shelf frame */}
      <mesh position={[0, 1.5, 0]}>
        <boxGeometry args={[4.5, 0.05, 1.5]}/>
        <meshStandardMaterial color="#334155" metalness={0.6} roughness={0.3}/>
      </mesh>
      {items.map((item, i) => (<StockBox key={item.product_id} item={item} position={[(i - items.length / 2 + 0.5) * 1.1, 0, 0]}/>))}
    </group>);
}
function Scene({ items }) {
    ...
    return (<>
      ...
      <WarehouseFloor />
      {rows.map((row, i) => (<ShelvingUnit key={i} items={row} offsetX={(i - rows.length / 2 + 0.5) * 5.5}/>))}
      <OrbitControls enablePan={false} minPolarAngle={Math.PI / 6} maxPolarAngle={Math.PI / 2.2} minDistance={8} maxDistance={20} autoRotate autoRotateSpeed={0.5}/>
    </>);
}
function generateDemoItems() {
    const statuses = ['healthy', 'low_stock', 'out_of_stock', 'overstock'];
...
export default function WarehouseScene({ items }) {
    return (<div className="w-full h-full rounded-2xl overflow-hidden">

...
export default function ProductsPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(Array.isArray(detail) ? detail.map((d: any) => d.msg).join(', ') : (detail ?? 'Failed to load products. Is the backend running?'))
      setItems([])
    } finally {
      setLoading(false)
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          <input
            className="px-3 py-2 rounded-xl glass border border-white/10 text-sm text-white placeholder-slate-500"
            placeholder="Search name / SKU / barcode..."
            value={q}
            onChange={(e) => setQ(e.target.value)}
          />
          <select
            className="px-3 py-2 rounded-xl glass border border-white/10 text-sm text-white bg-slate-900/50"
            value={categoryId}
      ...
      {categories.length === 0 && !loading && (
        <div className="glass rounded-2xl border border-amber-500/30 p-4 text-amber-200 text-sm">
      ...
      <form onSubmit={save} className="glass rounded-2xl border border-white/10 p-5 space-y-3">
        ...
        <div className="flex gap-2">
          ...
          {editingId && (
            <button type="button" onClick={resetForm} className="px-5 py-2 rounded-xl bg-slate-700 text-slate-300 text-sm">Cancel</button>
          )}
        </div>
      </form>
                ...
                <tr key={p.id} className="hover:bg-white/5">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      {p.image_path ? (
                        <img src={p.image_path} alt={p.name} className="h-10 w-10 rounded-lg object-cover border border-white/10" />
                      ) : (
                        <div className="h-10 w-10 rounded-lg bg-slate-800 border border-white/10" />
                      )}
                      <label className="text-xs px-2 py-1 rounded-lg bg-slate-700 text-slate-300 cursor-pointer hover:bg-slate-600">
                        Upload
                        <input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) uploadImage(p.id, f) }} />
                      </label>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex flex-col gap-1">
                      ...
                      {isAdmin && (
                        <button onClick={() => remove(p.id)} className="text-xs text-red-400 hover:text-red-300 text-left">Delete</button>
                      )}
                    </div>
                  </td>
                </tr>

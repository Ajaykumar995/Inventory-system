...
export default function ActivityPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const movementRows: ActivityRow[] = movements.data.items.map((m: StockMovement) => ({
        id: `m-${m.id}`,
        when: m.created_at,
      ...
      const saleRows: ActivityRow[] = sales.data.items.map((s: Sale) => ({
        id: `s-${s.id}`,
        when: s.sale_date,
        type: 'Sale',
      ...
      const merged = [...movementRows, ...saleRows].sort(
    ...
    <div className="space-y-6">
      ...
      <div className="glass rounded-2xl border border-white/10 overflow-hidden">
        ...
        <div className="divide-y divide-white/5 max-h-[600px] overflow-y-auto">
          {filtered.map((r) => (
            <div key={r.id} className="px-5 py-4 flex flex-wrap gap-4 items-center hover:bg-white/5">
              ...
              <span className={`text-xs px-2 py-1 rounded-lg shrink-0 ${
                r.type === 'Sale' ? 'bg-pink-500/20 text-pink-300' :
          ...
          {!loading && filtered.length === 0 && (
            <div className="px-5 py-10 text-center text-slate-500">No activity recorded yet.</div>
          )}
        </div>
      </div>
    </div>

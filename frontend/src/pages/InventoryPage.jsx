...
export default function InventoryPage() {
    ...
    const quickAction = async (productId, action, qty) => {
        setError('');
        try {
            if (action === 'receive')
                await inventoryService.receive(productId, { quantity: qty });
            else
                await inventoryService.issue(productId, { quantity: qty, reason: 'Issued' });
            await load();
        }
        catch (err) {
    ...
    return (<div className="space-y-6">
      ...
      <form onSubmit={setup} className="glass rounded-2xl border border-white/10 p-5 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          <select className="px-3 py-2 rounded-xl bg-slate-800/50 border border-white/10 text-sm" value={setupForm.product_id || ''} onChange={(e) => setSetupForm({ ...setupForm, product_id: Number(e.target.value) })} required>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {items.map((inv) => (<tr key={inv.id} className="hover:bg-white/5 transition">
              ...
              {!loading && items.length === 0 && (<tr><td colSpan={6} className="px-5 py-8 text-center text-slate-500">No inventory records. Setup stock for a product above.</td></tr>)}
            </tbody>
          </table>
        </div>
      </div>

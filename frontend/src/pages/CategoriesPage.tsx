...
export default function CategoriesPage() {
  ...
  const remove = async (id: number) => {
    if (!confirm('Delete this category? Products must be removed first.')) return
    setError('')
    try {
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={save} className="glass rounded-2xl border border-white/10 p-5 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <input
            className="px-3 py-2 rounded-xl bg-slate-800/50 border border-white/10 text-white placeholder-slate-500 text-sm"
          ...
          />
          <input
            className="px-3 py-2 rounded-xl bg-slate-800/50 border border-white/10 text-white placeholder-slate-500 text-sm md:col-span-2"
        ...
        <div className="flex gap-2">
          ...
          {editingId && (
            <button type="button" onClick={resetForm} className="px-5 py-2 rounded-xl bg-slate-700 text-slate-300 text-sm">
              Cancel
            </button>
          )}
        </div>
      ...
      <div className="flex gap-3">
        <input
          className="px-4 py-2 rounded-xl glass border border-white/10 text-sm text-white placeholder-slate-500 flex-1"
      ...
      <div className="glass rounded-2xl border border-white/10 overflow-hidden">
        ...
        <div className="divide-y divide-white/5">
          {items.map((c) => (
            <div key={c.id} className="px-5 py-4 flex items-center justify-between gap-4 hover:bg-white/5">
              <div>
              ...
              <div className="flex gap-2 shrink-0">
                ...
                {isAdmin && (
                  <button onClick={() => remove(c.id)} className="px-3 py-1.5 rounded-lg bg-red-600/20 text-red-300 text-xs hover:bg-red-600/40">
                    Delete
                  </button>
                )}
              </div>
          ...
          {!loading && items.length === 0 && (
            <div className="px-5 py-8 text-center text-slate-500 text-sm">No categories found.</div>
          )}
        </div>
      </div>
    </div>

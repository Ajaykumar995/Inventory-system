...
export default function PredictionPage() {
    ...
    if (loading)
        return <div className="text-slate-400 animate-pulse p-8">Analyzing sales patterns...</div>;
    return (<div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <select className="px-4 py-2 rounded-xl glass border border-white/10 text-sm" value={days} onChange={(e) => setDays(Number(e.target.value))}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        ].map((c) => (<div key={c.label} className="glass rounded-2xl border border-white/10 p-5">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="glass rounded-2xl border border-white/10 p-5">
          ...
          <div className="space-y-3 max-h-80 overflow-y-auto">
            {data?.reorder_recommendations.map((r) => (<div key={r.product_id} className="p-4 rounded-xl bg-slate-800/40 border border-white/5">
            ...
            {!data?.reorder_recommendations.length && (<p className="text-slate-500 text-sm text-center py-4">All stock levels healthy — no reorders needed</p>)}
          </div>
        </div>
          ...
          <div className="overflow-x-auto max-h-80 overflow-y-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-white/5">
                {data?.next_month_forecast.map((f) => (<tr key={f.product_id}>
                    <td className="py-2">
      ...
      <div className="glass rounded-2xl border border-white/10 p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {data?.slow_movers.map((p) => (<div key={p.product_id} className="p-4 rounded-xl bg-amber-500/5 border border-amber-500/20">

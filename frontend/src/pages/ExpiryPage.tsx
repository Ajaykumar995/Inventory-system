...
import type { InventoryBatch, ExpirySummary } from '../types/expiry'
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={c.label} className={`glass rounded-2xl border ${c.border} p-5 text-center`}>
      ...
      <div className="flex gap-2">
        {['', 'expiring_soon', 'expired', 'valid'].map((s) => (
          <button key={s} onClick={() => setFilter(s)} className={`px-4 py-2 rounded-xl text-sm transition ${filter === s ? 'bg-amber-600/30 border border-amber-500/30 text-amber-300' : 'glass border border-white/10 text-slate-400'}`}>
            {s ? s.replace('_', ' ') : 'All'}
          </button>
        ))}
      </div>
          ...
          <tbody className="divide-y divide-white/5">
            {batches.map((b) => (
              <tr key={b.id} className="hover:bg-white/5">
                ...
                <td className="px-5 py-3">
                  {!b.is_disposed && b.quantity > 0 && (
                    <button onClick={() => dispose(b.id)} className="px-2 py-1 rounded-lg bg-red-500/20 text-red-300 text-xs hover:bg-red-500/30">Dispose</button>
                  )}
                </td>
              </tr>
            ))}

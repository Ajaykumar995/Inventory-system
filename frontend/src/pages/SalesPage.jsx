...
export default function SalesPage() {
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 glass rounded-2xl border border-white/10 p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 max-h-[420px] overflow-y-auto">
            ...
            return (<button key={p.id} onClick={() => addToCart(p)} disabled={stock <= 0} className="text-left p-4 rounded-xl bg-slate-800/50 border border-white/10 hover:border-pink-500/40 transition disabled:opacity-40 disabled:cursor-not-allowed">
        ...
        <div className="glass rounded-2xl border border-white/10 p-5 flex flex-col">
          ...
          <div className="flex-1 space-y-2 overflow-y-auto max-h-48 mb-4">
            {cart.map((c) => (<div key={c.product_id} className="flex justify-between items-center text-sm bg-slate-800/40 rounded-lg px-3 py-2">
                <div>
      ...
      {/* Invoice modal */}
      {invoice && (<div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setInvoice(null)}>
          <div className="glass rounded-2xl border border-white/20 p-8 max-w-md w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-2 mb-4">
              {invoice.items.map((item) => (<div key={item.id} className="flex justify-between text-sm">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {sales.map((s) => (<tr key={s.id} className="hover:bg-white/5 cursor-pointer" onClick={() => setInvoice(s)}>

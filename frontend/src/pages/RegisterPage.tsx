...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    catch { setError('Registration failed. Email or username may already exist.') }
    ...
    <div className="min-h-screen gradient-mesh flex items-center justify-center relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md mx-4">
        ...
        <form onSubmit={handleSubmit} className="glass rounded-2xl p-8 space-y-4 border border-white/10">
          ...
          {(['full_name', 'email', 'username'] as const).map((f) => (
            <input key={f} className="w-full px-4 py-3 rounded-xl bg-slate-800/50 border border-white/10 text-white placeholder-slate-500 focus:border-blue-500 focus:outline-none" placeholder={f.replace('_', ' ')} value={form[f]} onChange={(e) => setForm({ ...form, [f]: e.target.value })} required />
          ))}

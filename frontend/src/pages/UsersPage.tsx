...
const ROLES = ['admin', 'inventory_manager', 'store_manager', 'employee', 'supplier'] as const
...
export default function UsersPage() {
  ...
  const updateUser = async (id: number, patch: { role_name?: string; is_active?: boolean }) => {
    setError('')
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      {isAdmin && (
        <form onSubmit={createUser} className="glass rounded-2xl border border-white/10 p-5 space-y-3">
              ...
              {users.map((u) => (
                <tr key={u.id} className="hover:bg-white/5">
                  ...
                  <td className="px-4 py-3">
                    {isAdmin && u.id !== me?.id ? (
                      <select
                        className="px-2 py-1 rounded-lg bg-slate-800 border border-white/10 text-xs text-white"
                        value={u.role.name}
                        onChange={(e) => updateUser(u.id, { role_name: e.target.value })}
                      >
                        {ROLES.map((r) => <option key={r} value={r}>{formatRole(r)}</option>)}
                      </select>
                    ) : (
                      <span className={`text-xs px-2 py-1 rounded-lg ${roleBadge[u.role.name] ?? ''}`}>

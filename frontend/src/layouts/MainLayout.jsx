...
export default function MainLayout() {
    ...
    return (<div className="min-h-screen gradient-mesh text-slate-100 flex">
      ...
      <aside className="w-64 glass border-r border-white/10 flex flex-col fixed h-full z-20">
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          {visibleNav.map((item) => {
            ...
            return (<Link key={item.to} to={item.to} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm transition-all ${active
        ...
        <div className="p-4 border-t border-white/10 space-y-3">
          ...
          <button onClick={toggle} className="w-full px-4 py-2 rounded-xl glass text-sm text-slate-300 hover:text-white transition">
